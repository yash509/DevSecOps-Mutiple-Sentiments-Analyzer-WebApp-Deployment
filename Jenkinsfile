pipeline {
	...
	VAULT_URL = 'http://13.233.207.160:8200/' // Vault server URL
    }
	...
	stage("Debugging HashiCorp Vault Credentials") {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-Mutiple-Sentiments-Analyzer-WebApp-Deployment.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop msa | true"
                sh "docker rm msa | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name msa -p 5000:80 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service msa-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"msa\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
